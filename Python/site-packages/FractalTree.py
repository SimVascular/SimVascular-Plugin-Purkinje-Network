...
def Fractal_Tree_3D(param):
    ...
    Args:
        param (Parameters object): this object contains all the parameters that 
    ...
    # Read mesh
    mesh = Mesh(param.input_file_name)
    ...
    branches[last_branch] = Branch(mesh, 0, init_dir, init_tri, param.init_length, 0.0, 0.0, nodes,
        [0], int(param.init_length/param.l_segment))
    branches_to_grow=[]
    ...
    if param.Fascicles:
        ...
        for i_branch in range(len(param.fascicles_angles)):
            ...
            branches[last_branch]=Branch(mesh,branches[0].nodes[-1],branches[0].dir,branches[0].tri,param.fascicles_length[i_branch],angle,0.0,nodes,brother_nodes,int(param.fascicles_length[i_branch]/param.l_segment))
        ...
        for g in branches_to_grow:
            ...
            for j in range(2):
                ...
                if l<param.min_length:
                    l=param.min_length
                branches[last_branch]=Branch(mesh,branches[g].nodes[-1],branches[g].dir,branches[g].tri,l,angle,param.w,nodes,brother_nodes,int(param.length/param.l_segment))
                #Add nodes to IEN
                for i_n in range(len(branches[last_branch].nodes)-1):
                    ien.append([branches[last_branch].nodes[i_n],branches[last_branch].nodes[i_n+1]]) 
    ...
    if param.save:
        if param.save_paraview:
            ...
            for i in range(len(nodes.nodes)):
                xyz[i,:]=nodes.nodes[i]                    
            write_line_VTU(xyz, ien, param.output_file_name + '.vtu')                
        
        np.savetxt(param.output_file_name +'_ien.txt',ien,fmt='%d')
