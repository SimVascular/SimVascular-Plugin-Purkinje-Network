...
def Fractal_Tree_3D(param):
    ...
    if param.save:
        if param.save_paraview:
            ## [Vijay] from ParaviewWriter import write_line_VTU
            from vtkWriter import writeLineVTU  ## [Vijay]
            logger.info('Finished growing, writing paraview file')
            xyz=np.zeros((len(nodes.nodes),3))
            for i in range(len(nodes.nodes)):
                xyz[i,:]=nodes.nodes[i]                    
            ## [Vijay] write_line_VTU(xyz, ien, param.output_file_name + '.vtu')
            writeLineVTU(xyz, ien, param.output_file_name + '.vtu')  ## [Vijay]
